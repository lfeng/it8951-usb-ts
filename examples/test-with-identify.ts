/**
 * Test to check if identify() causes the issue
 */
...
async function main() {
  console.log("=== Test with identify() call ===\n");
  logHardwareUsage("examples/test-with-identify.ts");
  ...
  try {
    ...
    // This is the difference - EPD.init() calls identify() before getDeviceInfo()
    console.log("Calling identify()...");
    const identifyResult = await usb.identify();
    console.log(
      `Identify result: isIT8951=${identifyResult.isIT8951}, vendor=${identifyResult.vendorId}, product=${identifyResult.productId}`,
    );
